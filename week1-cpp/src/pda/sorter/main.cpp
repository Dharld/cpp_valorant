...
void usage(const char* prog, int code = 0) {
  std::cerr << "Usage: " << prog << " [--unique] [--reserve=N]\n"
               "       " << prog << " [--unique] [--reserve N]\n";
  std::exit(code);
}
...
Options parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "-h" || a == "--help") usage(argv[0], 0);
            ...
            if (i + 1 >= argc) throw std::invalid_argument("--reserve needs a value");
...
int main(int argc, char** argv) {
    ...
    if (opts.reserve.has_value()) {
        size_t value = opts.reserve.value();
        arr.reserve(value);
    }
    ...
    for (auto it = arr.begin(); it != arr.end(); ++it) {
        std::cout << *it << '\n';   // one per line; '\n' avoids extra flush of std::endl
    }
